...
| 生命周期 | spawn → 执行 → 上报完成 → 挂起 → 等待父节点决策 |
| 通信 | `report_to_parent` 主动上报 · `send_message_to_agent` 同级通信 |

### 3.3 Review Agent（审查代理）

由 Expert 在 TaskBoard 全部完成后 spawn。

| 检查 | 方法 |
|------|------|
| **完整性** | TaskBoard 每个 task 都标记完成 |
| **一致性** | git diff 修改文件 == TaskBoard 声明的 file_targets |
| **正确性** | 测试全部通过 |

审查报告提交给 Core，由 Core 决定是否对外返回 Shell。
...
通过 `send_message_to_agent` / `read_agent_messages` 工具，**Expert 中转路由**。

所有同级消息必须经 Expert 中转——Expert 可监控通信质量。
