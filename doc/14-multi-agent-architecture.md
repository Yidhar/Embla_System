...
└── core/dna/
    ├── conversation_style_prompt.md     # 全局表达编排 DNA（不可修改）
    └── agentic_tool_prompt.md           # 全局工具调用契约 DNA（不可修改）
```
...
```
system/prompts/
├── core/routing/        # 路由分析 / 调度 prompt
├── agents/shell/blocks/ # Shell runtime 规则块
├── agents/core_exec/blocks/
├── roles/               # 角色定义块
├── skills/              # 技能描述块
├── styles/              # 输出风格块
└── rules/               # 规范约束块
...
**组装方式**：Expert spawn Dev 时从原子块池中挑选组装，并自动注入 L1 检索到的经验块。Shell/Core 主链则使用受控 DNA + runtime blocks 的固定分层。

**修改权限**：

| Agent | 身份/风格/工具 DNA | Runtime / 角色 / 技能 / 风格块 |
|-------|----------------------|-------------------------------|
| Core | ❌ 不可直接修改 | ✅ 可修改（受 registry / ACL / 审批链约束）|
| Expert | ❌ | ✅ 域内块 |
...
| `destroy_child_agent` | 销毁，释放资源，并返回 box/workspace 清理事实 |
...
| `publish_task_heartbeat` | 发布任务级 heartbeat，供 Expert/Core 做 stale 监管与阻断升级 |
...
### 6.7 Heartbeat 监管（运行时 canonical）

- `Dev` 在长任务、detached 执行或沙箱内操作时，持续调用 `publish_task_heartbeat` 上报 `task_id / stage / ttl_seconds / message`。
- `Expert` 以 `warning → critical → blocked` 的等级监管子任务：
  - `warning`：提醒原 Dev 刷新 heartbeat；
  - `critical`：注入更强恢复指令，要求说明阻塞原因；
  - `blocked`：优先 `heartbeat_respawn`，为同一 TaskBoard task 拉起 fresh Dev。
- 若 `heartbeat_respawn` 预算耗尽，Expert 直接上报 `expert_blocked(reason=task_heartbeat_blocked_respawn_exhausted)`。
- `Core` 在最终 `execution_receipt.agent_state` 中统一汇总 `heartbeat_summary`、`experts_blocked` 与 `blocked_expert_reasons`。
- `Shell` / `Core` 会话态通过 `/v1/chat/route_session_state/{session_id}` 读取当前子树 heartbeat 快照；运维总览通过 `/v1/ops/workflow/events` 读取 runtime 聚合态。
