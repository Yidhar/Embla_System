...
## 0. OS-sandbox-first 执行边界（Target Canonical）

自维护任务的手脚层 target canonical 不再是“宿主直接执行 + 路径重写补丁”，也不再把 `BoxLite` 作为默认执行面，而是：

- 宿主继续负责 `git worktree` 生命周期与 `audit/promote/teardown`。
- Dev / Review 的文件读写、搜索、命令执行、测试、lint、事务写入默认在宿主 OS 上、受 worktree 根边界约束的 `os_sandbox` 内完成。
- `native_tools` 需要重构为 backend router，按 session 的 `SandboxContext` 选择 `NativeExecutionBackend`、`OsSandboxExecutionBackend` 或 `BoxLiteExecutionBackend`。
- `apply_workspace_path_overrides` 从主路径补丁演进为 `os_sandbox` 的基础路径绑定层；host-only `native` 仅保留 system fallback 语义。
...
- `OsSandboxExecutionBackend`：作为自维护任务默认执行后端
- `BoxLiteManager` / `BoxLiteExecutionBackend`：作为可选强隔离执行面

详细设计见 `doc/16-os-sandbox-default-execution-architecture.md`；`BoxLite` 强隔离路径见 `doc/15-boxlite-first-execution-sandbox-architecture.md`。
