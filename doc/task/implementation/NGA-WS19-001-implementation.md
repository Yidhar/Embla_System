...
3. 测试覆盖
- 新增 `tests/test_agent_roles_ws30_005.py`
  - 验证典型故障目标可拆解为可派发子任务。
  - 验证分发过程遵守依赖顺序。
  - 验证反思结果与恢复入口可重建运行态。

## 验证命令
- `python -m ruff check agents/meta_agent.py autonomous/__init__.py tests/test_agent_roles_ws30_005.py`（归档路径，仅用于历史追溯）
...
- `evidence_link`:
  - `agents/meta_agent.py; autonomous/__init__.py [archived]; tests/test_agent_roles_ws30_005.py; doc/task/implementation/NGA-WS19-001-implementation.md`
- `notes`:
