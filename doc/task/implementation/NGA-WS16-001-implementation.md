...
| **Autonomous** | `autonomous/`（archived） | 📦 归档 | 否 | - | 历史 System Agent 自治循环（已迁移至 `agents/*`/`core/*`） |
...
- `autonomous/tools/cli_adapter.py`（archived） - CLI 统一适配器
- `autonomous/tools/codex_adapter.py`（archived） - Codex 主执行器
