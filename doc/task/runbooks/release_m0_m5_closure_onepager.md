...
| M3 | GC 证据链与预算守门生效 | `WS15-*`、`WS19-001..006/008` done；`tests/test_gc_*`、`tests/test_memory_agents.py`、`tests/test_chat_route_quality_guard_ws28_012.py` 通过 | 通过 |
...
`quick-mode` 为轻量门禁：跳过重回归、长稳演练与 Phase3/M8 Gate；`full` 执行完整 M0-M8 收口链。
...
5. T4 Autonomous 核心回归  
   `python -m pytest -q tests/test_core_event_bus_consumers_ws28_029.py tests/test_slo_snapshot_export.py tests/test_agent_roles_ws30_005.py tests/test_router_engine_prompt_profile_ws28_001.py tests/test_llm_gateway_prompt_slice_ws28_002.py tests/test_memory_agents.py tests/test_memory_agents.py tests/test_chat_route_quality_guard_ws28_012.py tests/test_core_event_bus_consumers_ws28_029.py tests/test_canary_rollback_drill.py -p no:tmpdir`
6. T5 发布运行工单产物  
