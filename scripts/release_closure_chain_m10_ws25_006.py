#!/usr/bin/env python3
"""WS25-006 M10 release closure chain runner."""
...
DEFAULT_EVENT_GC_REPORT = Path("scratch/reports/ws25_event_gc_quality_baseline.json")
DEFAULT_M10_GATE_REPORT = Path("scratch/reports/ws25_m10_closure_gate_result.json")
    ...
    *,
    ...
    if not skip_tests:
        steps.append(
            ChainStep(
                step_id="T0",
                description="WS25 M10 targeted regression tests",
                command=[python_exe, "-m", "pytest", "-q", *M10_TEST_TARGETS, "-p", "no:tmpdir"],
            )
        )
    if not skip_runtime_checks:
        steps.append(
            ChainStep(
                step_id="T1",
                description="WS25-005 event/gc quality baseline",
                command=[
                    python_exe,
                    "scripts/run_event_gc_quality_baseline_ws25_005.py",
                    "--output",
                    str(DEFAULT_EVENT_GC_REPORT).replace("\\", "/"),
                    "--gc-iterations",
                    "3",
                ],
            )
    ...
    if not skip_gate:
        steps.append(
            ChainStep(
                step_id="T2",
                description="WS25-006 M10 closure gate validation",
                command=[
                    python_exe,
                    ...
                    str(DEFAULT_EVENT_GC_REPORT).replace("\\", "/"),
                    "--output-json",
                    str(DEFAULT_M10_GATE_REPORT).replace("\\", "/"),
                ],
            )
        )
...
def run_release_closure_chain_m10_ws25_006(
    *,
    repo_root: Path,
    output_file: Path,
    ...
    report: Dict[str, object] = {
        "task_id": "NGA-WS25-006",
        "scenario": "release_closure_chain_m10_ws25_006",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run WS25-006 M10 release closure chain")
...
def main() -> int:
    args = parse_args()
    report = run_release_closure_chain_m10_ws25_006(
