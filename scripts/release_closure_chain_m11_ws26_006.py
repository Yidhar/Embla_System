#!/usr/bin/env python3
"""WS26-006 M11 release closure chain runner."""
...
DEFAULT_RUNTIME_CHAOS_REPORT = Path("scratch/reports/ws26_m11_runtime_chaos_ws26_006.json")
DEFAULT_M11_GATE_REPORT = Path("scratch/reports/ws26_m11_closure_gate_result.json")
    ...
    *,
    ...
    if not skip_tests:
        steps.append(
            ChainStep(
                step_id="T0",
                description="WS26 M11 targeted regression tests",
                command=[python_exe, "-m", "pytest", "-q", *M11_TEST_TARGETS, "-p", "no:tmpdir"],
            )
        )
    if not skip_runtime_checks:
        steps.append(
            ChainStep(
                step_id="T1",
                description="WS26-002 runtime snapshot export",
                command=[
                    python_exe,
                    "scripts/export_ws26_runtime_snapshot_ws26_002.py",
                    "--output",
                    str(DEFAULT_RUNTIME_SNAPSHOT_REPORT).replace("\\", "/"),
                ],
            )
        )
        steps.append(
            ChainStep(
                step_id="T2",
                description="WS26-006 M11 runtime chaos suite",
                command=[
                    python_exe,
                    "scripts/run_ws26_m11_runtime_chaos_suite_ws26_006.py",
                    "--output",
                    str(DEFAULT_RUNTIME_CHAOS_REPORT).replace("\\", "/"),
                ],
            )
        )
    if not skip_gate:
        steps.append(
            ChainStep(
                step_id="T3",
                description="WS26-006 M11 closure gate validation",
                command=[
                    python_exe,
                    ...
                    str(DEFAULT_RUNTIME_CHAOS_REPORT).replace("\\", "/"),
                    "--output-json",
                    str(DEFAULT_M11_GATE_REPORT).replace("\\", "/"),
                ],
            )
        )
...
def run_release_closure_chain_m11_ws26_006(
    *,
    repo_root: Path,
    output_file: Path,
    ...
    report: Dict[str, object] = {
        ...
        "scenario": "release_closure_chain_m11_ws26_006",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run WS26-006 M11 release closure chain")
    parser.add_argument("--repo-root", type=Path, default=Path("."), help="Repository root")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("scratch/reports/release_closure_chain_m11_ws26_006_result.json"),
        help="Output JSON report path",
    )
    parser.add_argument("--skip-tests", action="store_true", help="Skip T0 targeted tests")
    parser.add_argument("--skip-runtime-checks", action="store_true", help="Skip T1/T2 runtime report steps")
    parser.add_argument("--skip-gate", action="store_true", help="Skip T3 closure gate validation")
    parser.add_argument("--skip-doc-consistency", action="store_true", help="Skip T4 doc consistency")
