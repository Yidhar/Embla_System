...
def workspace_tmp_path() -> Path:
    root = Path("scratch") / "pytest_tmp_ws17_guard"
...
class TestTestBaselineGuard:
    ...
    def test_is_golden_suite_path(self, workspace_tmp_path):
        ...
        # 非 Golden Suite 路径
        workspace_path = workspace_tmp_path / "tests" / "test_example.py"
    ...
    def test_check_modification_allowed_workspace(self, workspace_tmp_path):
        """测试工作区测试文件修改（允许）"""
        config = TestBaselineConfig.default(project_root=workspace_tmp_path)
        guard = TestBaselineGuard(config)

        workspace_test = workspace_tmp_path / "tests" / "test_example.py"
        allowed, reason = guard.check_modification_allowed(workspace_test)

        assert allowed is True
        assert "Workspace test file" in reason

    def test_check_modification_blocked_golden(self, workspace_tmp_path):
    ...
    def test_check_modification_with_whitelist(self, workspace_tmp_path):
        """测试审批白名单（允许）"""
        config = TestBaselineConfig.default(project_root=workspace_tmp_path)
        guard = TestBaselineGuard(config)
    ...
    def test_validate_test_changes(self, workspace_tmp_path):
        """测试批量验证"""
        config = TestBaselineConfig.default(project_root=workspace_tmp_path)
        guard = TestBaselineGuard(config)
...
"""
    ...
    def test_analyze_test_file(self, workspace_tmp_path):
        """测试文件分析"""
        detector = TestPoisoningDetector()

        test_file = workspace_tmp_path / "test_example.py"
...
class TestCheckTestBaselineCompliance:
    ...
    def test_compliance_check_passed(self, workspace_tmp_path):
        """测试合规检查通过"""
        # 创建工作区测试文件
        test_file = workspace_tmp_path / "tests" / "test_example.py"
...
""")
        ...
        def patched_default(project_root=None):
            return original_default(project_root=workspace_tmp_path)
    ...
    def test_compliance_check_poisoning_detected(self, workspace_tmp_path):
        """测试毒化模式检测"""
        # 创建带毒化模式的测试文件
        test_file = workspace_tmp_path / "tests" / "test_poisoned.py"
        test_file.parent.mkdir(parents=True, exist_ok=True)
        test_file.write_text("""
def test_poisoned():
    assert True  # 弱化断言
