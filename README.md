...
源码：[`apiserver/llm_service.py`](apiserver/llm_service.py)、[`agents/tool_loop.py`](agents/tool_loop.py)（canonical；`apiserver/agentic_tool_loop.py` 为兼容 shim）、[`apiserver/native_tools.py`](apiserver/native_tools.py)
...
│   ├── agentic_tool_loop.py  #   兼容 shim → agents/tool_loop.py
...
│   ├── memory/           #   三维记忆子系统 (Working/Episodic/Semantic)
