...
from word_document_server.utils.file_utils import check_file_writeable, ensure_docx_extension, create_document_copy, resolve_document_path, validate_save_path
...
def create_document(filename: str, title: Optional[str] = None, author: Optional[str] = None, save_path: Optional[str] = None) -> str:
    ...
    # 如果指定了保存路径，先验证路径
    if save_path:
        is_valid, error_message = validate_save_path(save_path)
        if not is_valid:
