...
class MCPScheduler:
        ...
        try:
            ...
            for tool_call in task.tool_calls:
                try:
                    ...
                    results.append({"tool": tool_call.get("tool_name", "unknown"), "success": False, "error": str(e)})
            ...
            task.completed_at = datetime.utcnow().isoformat() + "Z"
            task.error = str(e)
            task.result = {"success": False, "error": str(e), "message": f"任务执行失败: {str(e)}"}
            # 回调失败也尝试通知
            try:
                await self._maybe_callback(task)
            except Exception:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        # 增加超时时间以等待LLM生成回复 (5秒 -> 60秒)
                        timeout = aiohttp.ClientTimeout(total=60)
                        async with session.post(callback_url, json=payload, timeout=timeout) as response:
                            ...
                            if response.status == 200:
                                ...
                                logger.warning(
                                    f"[回调调度] 工具结果回调失败 (尝试 {attempt + 1}/{max_retries}): 状态码={response.status}, 响应={response_text[:200]}"
                                )
    ...
    async def schedule_task(self, task_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "message": "任务已加入调度队列", "task_id": task.id}

        except Exception as e:
            logger.error(f"任务调度失败: {e}")
            return {"success": False, "error": str(e), "message": f"任务调度失败: {str(e)}"}
    ...
    async def check_duplicate(self, query: str, tool_calls: List[Dict[str, Any]]) -> Tuple[bool, Optional[str]]:
        ...
        for task_id, task in self.active_tasks.items():
            if (
                task.query == query
                and len(task.tool_calls) == len(tool_calls)
                and all(tc.get("tool_name") == tc2.get("tool_name") for tc, tc2 in zip(task.tool_calls, tool_calls))
            ):
                return True, task_id
